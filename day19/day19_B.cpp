...
int scanner(){
  ...
  for(int i=0;i<offsets.size()-1;i++){
    for(int j=i+1;j<offsets.size();j++){
      triplet r = substractTriplets(offsets[i],offsets[j]);
      int x = abs(std::get<0>(r));
      int y = abs(std::get<1>(r));
      int z = abs(std::get<2>(r));
      if(x+y+z > maxDistanceApart)
        maxDistanceApart = x+y+z;
    }
  }
  return maxDistanceApart;
}
int main(){
