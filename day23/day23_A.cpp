...
bool isAmphInPlace(int location,std::vector<Amphipod> amphs){
  for(int i=0;i<8;i++){
...
bool canMoveRoom(int index, std::vector<Amphipod> &amphs){
  ...
  if(isCaveClear(roomLoc,1,amphs) && isCaveClear(roomLoc,2,amphs) && isWayClear(m.location,roomLoc,amphs)){
  ...
  else if(isCaveClear(roomLoc,1,amphs) && isAmphInPlace(roomLoc,amphs) && isWayClear(m.location,roomLoc,amphs)){
    amphs[index].location = roomLoc;
    amphs[index].depth = 1;
    amphs[index].inPosition = true;
    amphs[index].inRoom = true;
